...
(function () {
  ...
  const BASE_API      = _scriptTag ? (_scriptTag.getAttribute('data-api')         || 'https://lidan.co.id/api') : 'https://lidan.co.id/api';
  const SIGNAL_PATH   = _scriptTag ? (_scriptTag.getAttribute('data-signal-path') || 'signal')                  : 'signal';
  const JOIN_PATH     = _scriptTag ? (_scriptTag.getAttribute('data-join-path')   || 'join-room')               : 'join-room';
  ...
  async function pollSignal() {
    if (!roomData) return;
    try {
      const res  = await fetch(`${API_SIGNAL}?room=${encodeURIComponent(roomData.room)}&participant=${encodeURIComponent(myName)}`);
      const data = await res.json();
